...
## Mantzoutsos Enzo et Guillot Sam
...
Le code est plus ou moins structuré en couche avec un repositor et un controller web, cependant, le découpage n'est pas complet car on trouve de la logique métier dans le controller web (au lieu de la trouver dans un domaine), et le fichier modèle sert à la fois à déclarer les entités JPA et les DTOS.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Cette règle se trouve dans le controller Web.

*Cette règle est-elle facilement testable par un test unitaire ?*

Cette règle est difficilement testable car elle est privée, et que son fonctionnement s'appuie sur instant.Now et donc du temps du serveur, ce qui impacte directement la robustesse des tests.
...
-it : Garde l'entrée standard ouverte et aloue un terminal pour la session
-e : Définit une variable d'environnement dans le conteneur (ici le mot de passe postgress)
-p : Redirection de port entre notre machine et le conteneur

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*

todoItemRepository est null car il y a un constructeur vide. Il faut supprimer ce constructeur et ajouter @Inject au-dessus du bon constructeur pour l'injection.
...
L'idée c'est rendre indépendante la partie métier / domaine de l'applicatiion des autres composants de l'architecture. On trouve donc le domaine au coeur de l'architecture, des output et input ports qui consistent en des interfaces, des contrats, et qui sont les seules portes d'entré et de sortie du domaine, et des adaptateur qui implémentent ces interfaces pour intéragire avec le domaine.
...
Pour les adaptateurs, c'est les tests d'intégration qu'il faut implémenter.
